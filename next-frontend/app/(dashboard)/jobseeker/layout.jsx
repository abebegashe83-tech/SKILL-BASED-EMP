...
const IconSkills = () => <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z"/></svg>;
const IconJobs = () => <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>;
const IconApplied = () => <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4"/></svg>;
...
const navItems = [
  ...
  { id: 'skills', label: 'My Skills', icon: <IconSkills /> },
  { id: 'jobs', label: 'Job Recommendations', icon: <IconJobs /> },
  { id: 'applied', label: 'Applied Jobs', icon: <IconApplied /> },
  { id: 'browse', label: 'Browse Jobs', icon: <IconSearch /> },
...
export default function JobseekerLayout({ children }) {
  ...
  useEffect(() => {
    ...
    if (session) {
      ...
      if (role !== 'jobseeker') {
        const target = role === 'employer' ? '/employer/dashboard' : '/login';
        router.push(target);
      } else {
        setUser(session);
      }
  ...
  const navigateToTab = (id) => {
    ...
    // If we are not on the dashboard page, navigate there first with the hash
    if (!window.location.pathname.includes('/jobseeker/dashboard')) {
      router.push(`/jobseeker/dashboard#${id}`);
    } else {
  ...
  const displayProfile = {
    ...
    initials: (user.fullName || user.name || 'U').split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2),
        ...
        <div className="h-full flex flex-col p-4">
          ...
          <div className="mb-8 px-3">
             <div className="flex items-center gap-3 p-4 rounded-2xl bg-gray-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-700">
                <div className="w-11 h-11 rounded-full bg-blue-600 flex items-center justify-center text-white font-bold text-sm overflow-hidden ring-2 ring-white dark:ring-gray-700 shadow-sm transition-transform hover:rotate-3">
                  {displayProfile.avatar ? <img src={displayProfile.avatar} alt="" className="w-full h-full object-cover"/> : displayProfile.initials}
                </div>
                <div className="flex-1 overflow-hidden">
                  <p className="text-sm font-bold text-gray-900 dark:text-white truncate">{displayProfile.name}</p>
                  <p className="text-[10px] uppercase tracking-wider font-semibold text-gray-400 truncate">{displayProfile.title}</p>
                </div>
             </div>
          </div>
          ...
          <div className="mt-auto space-y-1 pt-4 border-t border-gray-100 dark:border-gray-800">
            <ThemeToggle />
            <Link href="/jobseeker/profile" className="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 hover:text-blue-600 transition-all">
              <IconProfile />
      ...
      <div className="flex-1 lg:ml-64 flex flex-col min-h-screen">
        ...
        <header className="sticky top-0 z-40 bg-white/80 dark:bg-gray-950/80 backdrop-blur-md border-b border-gray-100 dark:border-gray-800 h-16 flex items-center px-8 justify-between transition-colors">
          <div className="flex items-center gap-4 flex-1">
            ...
            <div className="hidden md:flex items-center gap-3 bg-gray-100 dark:bg-gray-800 rounded-2xl px-5 py-2.5 w-full max-w-md border border-transparent focus-within:border-blue-500/50 focus-within:bg-white dark:focus-within:bg-gray-900 focus-within:shadow-lg focus-within:shadow-blue-500/5 transition-all">
               <span className="text-gray-400"><IconSearch /></span>
               <input type="text" placeholder="Search for jobs, companies, skills..." className="bg-transparent border-none focus:ring-0 text-sm w-full text-gray-900 dark:text-white placeholder:text-gray-400" />
            </div>
          </div>
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-3 pl-6 border-l border-gray-100 dark:border-gray-800">
                <div className="hidden sm:block text-right">
                    <p className="text-xs font-black text-gray-900 dark:text-white leading-none mb-1">{displayProfile.name}</p>
                    <div className="flex items-center justify-end gap-1">
                      <span className="w-1.5 h-1.5 bg-green-500 rounded-full"></span>
                      <p className="text-[10px] font-bold text-gray-400">ONLINE</p>
                    </div>
                </div>
                <Link href="/jobseeker/profile" className="w-10 h-10 rounded-full bg-blue-600 border-2 border-white dark:border-gray-800 shadow-md flex items-center justify-center text-white font-bold text-xs overflow-hidden transition-transform hover:scale-105 active:scale-95">
                   {displayProfile.avatar ? <img src={displayProfile.avatar} alt="" className="w-full h-full object-cover"/> : displayProfile.initials}
                </Link>
            </div>
          </div>
